...
#include <torch/csrc/opencl/device_set.h>
...
std::vector<Tensor> broadcast(const Tensor& tensor, IntArrayRef devices) {
  if (tensor.is_opencl() && tensor.get_device() != devices[0])
  ...
  tensors.reserve(devices.size());
  {
    ...
    IntArrayRef loop_devices = tensor.is_opencl() ? devices.slice(1) : devices;
    for (auto device : loop_devices) {
      tensors.push_back(tensor.to(
          at::Device(kOPENCL, device),
          tensor.scalar_type(),
          /*non_blocking=*/true,
          /*copy=*/true));
    }
...
tensor_list2d broadcast_coalesced(TensorList tensors, IntArrayRef devices, size_t buffer_size) {
  ...
  c10::opencl::OpenCLGuard device_guard(devices[0]);
...
std::vector<at::Tensor> scatter(
    ...
    int64_t dim,
    const c10::optional<std::vector<c10::optional<c10::opencl::OpenCLStream>>>& streams) {
  ...
  c10::opencl::OptionalOpenCLStreamGuard opencl_guard;
  for (size_t chunk = 0; chunk < chunks.size(); ++chunk) {
    ...
    if (streams && (*streams)[chunk]) {
      ...
      opencl_guard.reset_stream(*(*streams)[chunk]);
    }
    chunks[chunk] = chunks[chunk].contiguous().to(
        {at::DeviceType::OPENCL, device_index}, /*non_blocking=*/true);
  }
  return chunks;
}
...
at::Tensor gather(
  ...
  for (const auto& tensor : tensors) {
    TORCH_CHECK(
        tensor.is_opencl(), "Gather expects all inputs to have OpenCL type");
  ...
  if (!destination_index || *destination_index != -1) {
    device = at::Device(at::DeviceType::OPENCL, destination_index ? *destination_index : -1);
  }
