...
static void pointwise_op3(TensorImpl* c, const TensorImpl* a, const TensorImpl* b, at::native::opencl::OpenCLOperations op) {
  ...
  pointwise_op_f.setArg<cl_mem>(0, (*(cl::Buffer*)a->storage().unsafeGetStorageImpl()->data_ptr().get())());pointwise_op_f.setArg<cl_mem>(1, (*(cl::Buffer*)b->storage().unsafeGetStorageImpl()->data_ptr().get())());
  pointwise_op_f.setArg<cl_mem>(2, (*(cl::Buffer*)c->storage().unsafeGetStorageImpl()->data_ptr().get())());
  pointwise_op_f.setArg<at::native::opencl::OpenCLOperations>(3, op);
...
static void pointwise_op(TensorImpl* b, const TensorImpl* a, at::native::opencl::OpenCLOperations op) {
  // DONE Call OpenCL kernel.
  ...
  cl::Kernel pointwise_op_f = opt_kernel.value();
  pointwise_op_f.setArg<cl_mem>(0, (*(cl::Buffer*)a->storage().unsafeGetStorageImpl()->data_ptr().get())());pointwise_op_f.setArg<cl_mem>(1, (*(cl::Buffer*)b->storage().unsafeGetStorageImpl()->data_ptr().get())());
  pointwise_op_f.setArg<at::native::opencl::OpenCLOperations>(2, op);
  auto stream = caffe2::opencl::getCurrentOpenCLStream(a->device().index());
  cl::Event event;
  stream.stream()->enqueueNDRangeKernel(pointwise_op_f, 0, a->numel(), cl::NullRange, NULL, &event);
  event.wait();
...
static void logical_tensor(TensorImpl *self_, const TensorImpl *t1, const TensorImpl *t2, at::native::opencl::OpenCLOperations op) {
  opencl_resize(self_, t1->sizes(), {});
  TORCH_CHECK(opencl_nElement(t1) == opencl_nElement(t2), "sizes don't match");
  pointwise_op3(self_, t1, t2, op);
}
...
Tensor _eq_opencl(const Tensor &self, const Tensor& other) {
  ...
  logical_tensor(result_, self_, other_, at::native::opencl::OpenCLOperations::EQ);
...
Tensor _abs_out_opencl(const Tensor &self) {
  auto allocator = c10::GetAllocator(DeviceType::OPENCL);
  auto result_ = c10::make_intrusive<TensorImpl, UndefinedTensorImpl>(c10::Storage(scalarTypeToTypeMeta(self.scalar_type()), 0, allocator, true),TensorTypeId::OpenCLTensorId).release();
  auto result = Tensor(c10::intrusive_ptr<TensorImpl, UndefinedTensorImpl>::reclaim(result_));
  auto self_ = checked_tensor_unwrap(self, "self", 1, "_abs_out_opencl", false, c10::Backend::OpenCL, ScalarType::Float);

  opencl_resize(result_, self_->sizes(), {});
  pointwise_op(result_, self_, at::native::opencl::OpenCLOperations::ABS);
